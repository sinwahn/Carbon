...
int carbon_getdescriptorinfo(lua_State* L)
{
	...
	if (lua_isnoneornil(L, 2))
	{
		...
		lua_pushstring(L, "Event");
		pushCollection(instance->getDescriptor()->events, createEmptyInfo);
		lua_settable(L, -3);

		lua_pushstring(L, "Function");
		pushCollection(instance->getDescriptor()->functions, createEmptyInfo);
		lua_settable(L, -3);

		lua_pushstring(L, "YieldFunction");
		pushCollection(instance->getDescriptor()->yieldFunctions, createEmptyInfo);
		lua_settable(L, -3);
